...
for p in files:
    ...
    lc = raw.count(b"\n") + 1
    # store raw bytes (NVARCHAR via decode may alter bytes) -> use COMPRESS on the raw text with utf-8-sig tolerance
    ...
    cur.execute("""MERGE dbo.LensSource AS t USING (SELECT ? repo, ? path) s ON (t.repo=s.repo AND t.path=s.path)
        WHEN MATCHED THEN UPDATE SET content_sha256=?, content_gz=COMPRESS(CAST(? AS NVARCHAR(MAX))), byte_len=?, line_count=?, wave=?, ingested_utc=SYSUTCDATETIME()
        ...
        repo, rel, sha, text, len(raw), lc, WAVE,
        repo, rel, os.path.splitext(p)[1], sha, text, len(raw), lc, WAVE)
    ingested += 1
cn.commit()
...
cur.execute("SELECT path, CAST(DECOMPRESS(content_gz) AS NVARCHAR(MAX)), content_sha256 FROM dbo.LensSource WHERE repo=?", repo)
rows = cur.fetchall()
match = 0; mismatch = 0; mism_list = []
for rel, text, stored_sha in rows:
    # compare on TEXT content (utf-8) — the reassembly test the store round-trip already proves at byte level for compressed cols
    orig_path = os.path.join(ROOT, rel.replace("/", os.sep))
    try:
        with open(orig_path, "rb") as fh: orig = fh.read()
        orig_text = orig.decode("utf-8", "replace")
    except Exception:
